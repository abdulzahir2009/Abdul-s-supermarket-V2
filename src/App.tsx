...
export default function App() {
  ...
  function navigate(p: string, data?: unknown) {
    if (p === "product-detail" && data) setSelectedProduct(data as Product);
    setPage(p as Page);
  ...
  const showFooter = !["checkout", "whatsapp-confirm"].includes(page);
  ...
  const mobileNav = [
        ...
        <div className="relative">
          ...
          {cartCount > 0 && (
            <span className="absolute -top-1 -right-1 w-3.5 h-3.5 rounded-full text-white flex items-center justify-center"
              style={{ background: "#6AB04C", fontSize: 8, fontFamily: "Montserrat, sans-serif", fontWeight: 700 }}>
              {cartCount}
            </span>
          )}
        </div>
    ...
    { label: "About", p: "about", icon: <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10"/><line x1="12" y1="16" x2="12" y2="12"/><line x1="12" y1="8" x2="12.01" y2="8"/></svg> },
  ];
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ background: "#fff" }}>
      ...
      <main className="flex-1 pb-16 md:pb-0">
        ...
        {page === "about" && <AboutPage onNavigate={(p) => navigate(p)} />}
        {page === "whatsapp-confirm" && <WhatsAppConfirmPage onNavigate={(p) => navigate(p)} />}
      </main>
      ...
      >
        {mobileNav.map(({ label, p, icon }) => (
          ...
          >
            {icon}
            <span className="text-xs" style={{ fontFamily: "Montserrat, sans-serif", fontWeight: page === p ? 700 : 500, fontSize: 9 }}>
              {label}
            </span>
          </button>
        ))}
